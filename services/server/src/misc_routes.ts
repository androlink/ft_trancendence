...
export async function miscRoutes(fastifyInstance: FastifyInstance) {
...

  {
    ...
    const statement1 = db.prepare<{start: string}, {username: string, pfp: string}>("SELECT username, pfp FROM users WHERE lower(username) LIKE REPLACE(:start, '_', '\\_') || '%' ESCAPE '\\'");
...

  {
    ...
    const statement1 = db.prepare<{id: Id}, {username: string, pfp: string}>("SELECT u.username, u.pfp FROM friends f JOIN users u ON u.id = CASE WHEN f.friend_one = :id THEN f.friend_two ELSE f.friend_one END WHERE :id IN (f.friend_one, f.friend_two)");
    fastifyInstance.get<{Querystring: {page: string}}>('/friends', async (req, reply) => {
