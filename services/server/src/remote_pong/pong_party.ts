...
{
  const statement = db.prepare<{id: Id}, string>("SELECT username FROM  users WHERE id = :id");
  get_username = (id) =>
  {
...
export function pong_party_create(game_id: string, players_id: Id[]): GameParty {
...
export function pong_party_delete(game_id: string): void {
  ...
  p.players.forEach((p) => p.ws.close());
}
...
export function pong_party_add_player(game_id: string, id: Id, ws: WebSocket)
{
  ...
  (ws as GameWebsocket).oninput = (ev, message) => {
    ...
    if (message.input == "pressDown")
      player.down = true;
    if (message.input == "releaseDown")
      player.down = false;
    if (message.input == "pressUp")
      player.up = true;
    if (message.input == "releaseUp")
      player.up = false;
  };
  ws.onclose = (ev) => {
    player.ready = "GONE";
