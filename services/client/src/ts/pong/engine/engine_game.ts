...
function eventKeyInputPong(event: KeyboardEvent) {
  ...
  game.players.forEach((player) => {
    if (player.up.key === undefined || player.down.key === undefined) return;
...
function botKeyPressingPong() {
  ...
  for (const player of players) {
    ...
    if (
      (player.view.direction === "E" &&
        (player.bot_difficulty + 1) ** 3 * 0.8 <
          ball.view.x - (player.view.TL.x + player.view.width)) ||
      (player.view.direction === "W" &&
        (player.bot_difficulty + 1) ** 3 * 0.8 < player.view.TL.x - ball.view.x)
    ) {
    ...
    let estimated_collision =
      (((ball.view.y + ball_size / 2 + estmated_nb_call * dy) % 200) + 200) %
      200;
    if (estimated_collision > 100)
      estimated_collision = 200 - estimated_collision;
    if (
      estimated_collision === player.view.TL.y + player.view.height / 2 ||
...
function startLocalPong(): void {
  if (game.views.state === "waiting") game.views.state = "playing";
...
function resumeLocalPong() {
  if (game.views.state !== "paused") return;
  if (game.intervalId !== undefined) return;
  game.intervalId = setInterval(tick, delay, game);
